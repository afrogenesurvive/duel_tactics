export function processLevelData(app, allCells) {
  // console.log('processing level data','grid width',app.gridWidth);
  ...
  for (let elem of allCells) {
    ...
    if (
      ...
      (elemLevelData.split("_")[1] !== "*" &&
        app.terrainLevelDataRef[elemLevelData.split("_")[3]].type === "void")
    ) {
    ...
    elem.terrain = app.terrainLevelDataRef[elem.levelData.split("_")[3]];
    ...
    if (elem.levelData.split("_")[4].charAt(1) !== "*") {
      elem.elevation.type =
        app.elevationTypeLevelDataRef[elem.levelData.split("_")[4].charAt(1)];
    }
    ...
    if (elem.levelData.split("_")[1] !== "*") {
      elem.obstacle = JSON.parse(
        JSON.stringify(app.obstacleLevelDataRef[elem.levelData.split("_")[1]]),
      );
      ...
      elem.obstacle.trap = app.obstacleBarrierTrapInitSet("main", "obstacle", elem);
      obstacleCount++;
    }
    ...
    if (elem.levelData.split("_")[0] !== "**") {
      elem.barrier = JSON.parse(
        JSON.stringify(app.barrierLevelDataRef[elem.levelData.split("_")[0].charAt(0)]),
      );
      ...
      elem.barrier.trap = app.obstacleBarrierTrapInitSet("main", "barrier", elem);
      barrierCount++;
    }
    ...
    if (elem.number.x === app.gridWidth) {
      elem.edge = {
        state: true,
        side: "east",
      };
    }
    if (elem.number.y === app.gridWidth) {
      elem.edge = {
        state: true,
        side: "south",
      };
  ...
  obstacleCount = 0;
  barrierCount = 0;
  for (const elem2 of app.settingsGridInfo) {
    // SET LEVEL DATA!
    ...
    // TERRAIN
    elem2.terrain = app.terrainLevelDataRef[elem2.levelData.split("_")[3]];
    ...
    if (elem2.levelData.split("_")[4].charAt(1) !== "*") {
      elem2.elevation.type =
        app.elevationTypeLevelDataRef[elem2.levelData.split("_")[4].charAt(1)];
    }
    ...
    if (elem2.levelData.split("_")[1] !== "*") {
      elem2.obstacle = JSON.parse(
        JSON.stringify(app.obstacleLevelDataRef[elem2.levelData.split("_")[1]]),
      );
      ...
      elem2.obstacle.trap = app.obstacleBarrierTrapInitSet("main", "obstacle", elem2);
      obstacleCount++;
    }
    ...
    if (elem2.levelData.split("_")[0] !== "**") {
      elem2.barrier = JSON.parse(
        JSON.stringify(app.barrierLevelDataRef[elem2.levelData.split("_")[0].charAt(0)]),
      );
      ...
      elem2.barrier.trap = app.obstacleBarrierTrapInitSet("main", "barrier", elem2);
      barrierCount++;
    }
    ...
    if (elem2.number.x === app.settingsGridWidth) {
      elem2.edge = {
        state: true,
        side: "east",
      };
    }
    if (elem2.number.y === app.settingsGridWidth) {
      elem2.edge = {
        state: true,
        side: "south",
      };
  ...
  app.gridInfo2D = gridInfo2d;
  // console.log('gridInfo2d',app.gridInfo2D);
  app.gridInfo = allCells;

  // app.settingsFormAiGridInfo = app.gridInfo;
  // console.log('post parse gridInfo',app.gridInfo);

  // console.log('yyy',allCells.filter(x => x.barrier.state === true).map(y => y = y.barrier.position));
  // console.log('yyy2',app.settingsGridInfo.filter(x => x.barrier.state === true).map(y => y = y.barrier.position));

export function startProcessLevelData(app, canvas) {
  // console.log('startProcessLevelData',app.gridWidth);
  ...
  for (var x = 0; x < app.gridWidth + 1; x++) {
    for (var y = 0; y < app.gridWidth + 1; y++) {
      ...
      let center = {
        x: Math.round(iso.x - offset.x / 2 + app.cellCenterOffsetX),
        y: Math.round(iso.y - offset.y / 2 - app.cellCenterOffsetY),
      };
      ...
      gridInfo.push({
        ...
        vertices: [
          { x: center.x, y: center.y + app.tileWidth / 2 },
          { x: center.x + app.tileWidth, y: center.y },
          { x: center.x, y: center.y - app.tileWidth / 2 },
          { x: center.x - app.tileWidth, y: center.y },
        ],
        side: Math.sqrt((app.tileWidth / 2) ^ (2 + app.tileWidth) ^ 2),
        levelData: "",
        edge: {
          state: false,
          side: "",
  ...
  for (var x = 0; x < app.settingsGridWidth + 1; x++) {
    for (var y = 0; y < app.settingsGridWidth + 1; y++) {
      ...
      iso2.x += app.settingsSceneX;
      iso2.y += app.settingsSceneY;

      let center2 = {
        x: Math.round(iso2.x - offset2.x / 2 + app.cellCenterOffsetX / 2),
        y: Math.round(iso2.y - offset2.y / 2 - app.cellCenterOffsetY / 2),
      };
      ...
      settingsGridInfo.push({
        ...
        side: Math.sqrt((app.tileWidth / 2 / 2) ^ (2 + app.tileWidth / 2) ^ 2),
        levelData: "",
        edge: {
          state: false,
          side: "",
  ...
  app.settingsGridInfo = settingsGridInfo;
  app.gridInfo = gridInfo;
}
